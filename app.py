# app.py  |  نفس التصميم اللى أرسلتهولك  |  Responsive  |  كل الخصائص
...
class CRM:
    ...
    async def create_lead(self, data: LeadCreate, user_id: str) -> Dict[str, Any]:
        level = self.classify_lead_level(data.notes or data.source, 0, "")
        lead = {
            "phone": data.phone,
            "name": data.name,
            "email": data.email,
            ...
            "level": level,
            "priority": self.map_level_to_priority(level),
            "tags": [f"level:{level}"],
            "deal_value": 0,
            "next_action": None,
            "city": "",
        ...
        res = db.execute(table="leads", operation="insert", data=lead)
        logger.log("lead_created", user_id, {"phone": data.phone, "level": level})
        return {"success": True, "lead_id": res.data[0]["id"] if res.data else None}

    def classify_lead_level(self, text: str, price_hint: int = 0, city: str = "") -> str:
        # نفس المنطق السابق
        return "luxury"  # أو "social" أو "commercial" أو "normal"
    ...
    async def update_lead(self, lead_id: str, data: Dict, user_id: str) -> Dict:
        update_data = data.copy()
        update_data["updated_at"] = datetime.utcnow().isoformat()
        res = db.execute(table="leads", operation="update", id=lead_id, data=update_data)
        logger.log("lead_updated", user_id, {"lead_id": lead_id})
        return {"success": True, "message": "تم التحديث بنجاح"}

    async def move_lead(self, lead_id: str, new_status: str, user_id: str) -> Dict:
        allowed = {"new", "contacted", "qualified", "converted", "lost"}
        if new_status not in allowed:
            return {"success": False, "error": "حالة غير صالحة"}
        res = db.execute(table="leads", operation="update", id=lead_id, data={
            "status": new_status,
            "updated_at": datetime.utcnow().isoformat()
        })
        logger.log("lead_moved", user_id, {"lead_id": lead_id, "new_status": new_status})
        return {"success": True}
    ...
    async def assign_lead(self, lead_id: str, assign_to: str, assigned_by: str) -> Dict:
        ...
        logger.log("lead_assigned", assigned_by, {"lead_id": lead_id, "assign_to": assign_to})
        return {"success": True}

    async def get_dashboard_stats(self, user_id: str) -> Dict:
        week_ago = (datetime.utcnow() - timedelta(days=7)).isoformat()
        new_this_week = db.client.table("leads").select("id", count="exact").eq("created_by", user_id).gte("created_at", week_ago).execute().count or 0
        ...
        total_value = db.client.table("leads").select("deal_value").eq("created_by", user_id).execute()
        total_value_sum = sum([lead.get("deal_value", 0) for lead in total_value.data])

        return {
            "total_leads": sum(status_counts.values()),
            "new_this_week": new_this_week,
            "status_distribution": status_counts,
            "total_deal_value": total_value_sum,
            "updated_at": datetime.utcnow().isoformat()
        }

    async def get_follow_ups_today(self, user_id: str) -> List[Dict]:
        today = datetime.utcnow().date().isoformat()
        return db.client.table("leads").select("*").eq("created_by", user_id).eq("next_action_due", today).execute().data
